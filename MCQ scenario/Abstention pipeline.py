...
def data_process_for_MMLU(path1, path2, order_list, category, for_GMM=True):
    train_data = []
    test_data = []
...
models = ['llama2', 'llama3', 'GLM4']
datasets = ['MMLU', 'CSQA', 'ARC', 'MedQA', 'MedMCQA']
...
for dataset in datasets:
    ...
    R_accs = np.zeros((len(models), 3, num_order))
    A_F1s = np.zeros((len(models), 3, num_order))

    for model in models:
        prompt_id = prompt_id_dict[f'{model}-{dataset}']

        train_base_path = f'/Data/datasets/Option order results/MMLU/LLM results/{model}/prompt{prompt_id}_train_results_'
        test_base_path = f'/Data/datasets/Option order results/MMLU/LLM results/{model}/prompt{prompt_id}_test_results_'

        if dataset == 'MMLU':
            result_path = f'/Data/datasets/Option order results/MMLU/calibrated results/{model}/ALL_single_results.json'
        else:
            result_path = f'/Data/datasets/Option order results/{dataset}/calibrated results(coefs migration)/{model}/single_results.json'

        with open(result_path, 'r', encoding='utf-8') as f:
            result = json.load(f)

        threshold = result['thresholds']
        inconsistency_scores = result['inconsistent_scores']
        inconsistency_scores = np.max(inconsistency_scores, axis=1)

        if dataset == 'MMLU':
            train_data, test_data, train_labels, test_labels = data_process_for_MMLU(train_base_path, test_base_path, all_orders, 'ALL', for_GMM=False)
        else:
            train_data, test_data, train_labels, test_labels = data_process(train_base_path, test_base_path, all_orders, for_GMM=False)


        # The abstention strategy based on our pipeline
        incons_samples_GMM = np.where(inconsistency_scores >= threshold)[0]
        cons_samples_GMM = np.where(inconsistency_scores < threshold)[0]


        # The "Avg Conf" method
        mean_conf_on_train = np.max(np.mean(train_data, axis=1), axis=1)
        mean_conf_on_test = np.max(np.mean(test_data, axis=1), axis=1)

        # Determine the threshold of "Avg Conf"
        Avg_pre_on_train = np.argmax(np.mean(train_data, axis=1), axis=1)
        Avg_correctness_on_train = (Avg_pre_on_train == train_labels).astype(int)
        accept_rate = np.linspace(0, 1, 101)[1:]
        thresholds_iteration = np.percentile(mean_conf_on_train, accept_rate * 100)
        accs_conf_train = []
        for t in thresholds_iteration:
            cons_or_not = (mean_conf_on_train > t).astype(int)
            accs_conf_train.append(accuracy_score(Avg_correctness_on_train, cons_or_not))
        ts_conf = thresholds_iteration[np.argmax(accs_conf_train)]

        # Results of "Avg Conf" strategy
        conf_samples = np.where(mean_conf_on_test > ts_conf)[0]
        inconf_samples = np.where(mean_conf_on_test <= ts_conf)[0]

        #################################################################################################################################

        for k in range(train_data.shape[1]):
            # The golden labels for abstention task in the held-out set, which are dependent on specific option order
            raw_pre_on_train = np.argmax(train_data[:, k, :], axis=1)
            correctness_on_train = (raw_pre_on_train == train_labels).astype(int)

            # The "Conf" method
            conf_on_train = np.max(train_data[:, k, :], axis=1)
            conf_on_test = np.max(test_data[:, k, :], axis=1)

            # Determine the threshold of "Conf" method
            accept_rate = np.linspace(0, 1, 101)[1:]
            thresholds_iteration = np.percentile(conf_on_train, accept_rate * 100)
            accs_conf_train = []
            for t in thresholds_iteration:
                cons_or_not = (conf_on_train > t).astype(int)
                accs_conf_train.append(accuracy_score(correctness_on_train, cons_or_not))
            ts_conf_wo_perm = thresholds_iteration[np.argmax(accs_conf_train)]

            # Results of "Conf" strategy
            conf_samples_wo_perm = np.where(conf_on_test > ts_conf_wo_perm)[0]
            inconf_samples_wo_perm = np.where(conf_on_test <= ts_conf_wo_perm)[0]
            #######################################################################################################################################

            # The golden labels for abstention task in the validation set, which are dependent on specific option order
            raw_pre_on_test = np.argmax(test_data[:, k, :], axis=1)
            golden_answer = (raw_pre_on_test == test_labels).astype(int)

            abstention_GMM = np.zeros(test_data.shape[0])
            abstention_GMM[incons_samples_GMM] = 1

            abstention_conf_wo_perm = np.zeros(test_data.shape[0])
            abstention_conf_wo_perm[inconf_samples_wo_perm] = 1

            abstention_conf = np.zeros(test_data.shape[0])
            abstention_conf[inconf_samples] = 1

            cm_GMM = confusion_matrix(abstention_GMM, 1 - golden_answer)
            cm_conf_wo_perm = confusion_matrix(abstention_conf_wo_perm, 1 - golden_answer)
            cm_conf = confusion_matrix(abstention_conf, 1 - golden_answer)

            R_accs[models.index(model), 0, k] = cm_GMM[0, 0] / np.sum(cm_GMM[0, :])
            R_accs[models.index(model), 1, k] = cm_conf_wo_perm[0, 0] / np.sum(cm_conf_wo_perm[0, :])
            R_accs[models.index(model), 2, k] = cm_conf[0, 0] / np.sum(cm_conf[0, :])


            A_F1s[models.index(model), 0, k] = f1_score(1 - golden_answer, abstention_GMM)
            A_F1s[models.index(model), 1, k] = f1_score(1 - golden_answer, abstention_conf_wo_perm)
            A_F1s[models.index(model), 2, k] = f1_score(1 - golden_answer, abstention_conf)

    Racc_dict[dataset] = R_accs
    AF1_dict[dataset] = A_F1s
