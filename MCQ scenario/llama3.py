...
def build_history(knowledge_base:list,indices, letters, prompt_template):
    ...
    for indice in indices:
        labels = knowledge_base[indice]['choices'].tolist()
...
for exchange in orders_list:
    ...
    aim_token_ids_1 = torch.tensor([32, 33, 34, 35, 36]).to(model.device)  # ABCD有两套ids与之对应
    aim_token_ids_1 = aim_token_ids_1[:num_options]
    aim_token_ids_2 = torch.tensor([362, 426, 356, 423, 469]).to(model.device)
    ...
    if args.restart == 'n':
        ...
        save_path = f'/Data/datasets/Option order results/{args.dataset}/LLM results/llama3/prompt{args.prompt}_{args.subset}_results_{result_file_name}.jsonl'
        finished_num = 0
