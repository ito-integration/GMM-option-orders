...
model_path = '/Data/transformers/Llama3-instruction'
...
def construct_input(dialog, tokenizer, model):
    ...
    dialog_text = "<|begin_of_text|>"
    for message in dialog:
        dialog_text += f"<|start_header_id|>{message['role']}<|end_header_id|>\n\n{message['content'].strip()} <|eot_id|>"

    dialog_text += "<|start_header_id|>assistant<|end_header_id|>\n\n"

    dialog_tokens_1 = tokenizer([dialog_text], return_tensors="pt", add_special_tokens=False)
...
# fixed_shot_index = [492, 448, 0, 78]# 分别对应一种选项, 经验证，不在被采样的train集之中
...
for exchange in orders_list:
    ...
    aim_token_ids_2 = torch.tensor([362, 426, 356, 423, 469]).to(device)  # ABCD有两套ids与之对应
    ...
    if args.restart == 'n':
        ...
        finished_num = 0
        save_path = f'/Data/datasets/Option order results/{args.dataset}/LLM results/llama3/{args.subset}_results_{result_file_name}.json'
    ...
    for item in tqdm(data[finished_num:]):
        ...
        if (outputs[0] in aim_token_ids_1) or (outputs[0] in aim_token_ids_2):  # 如果输出的第一个字母是ABCD中的一个，则符合格式
            ...
            with torch.no_grad():
                outputs_set = model.generate(**inputs, do_sample=False, max_new_tokens=50,
                                             pad_token_id=-1,
                                             return_dict_in_generate=True, output_scores=True
                                             )
            outputs = outputs_set.sequences.tolist()[0][len(inputs["input_ids"][0]):]
            response = tokenizer.decode(outputs)
            print('The unformatted output: ', response)
            logits = outputs_set.scores
    ...
    for i in range(len(exchange)):
        result_file_name += str(exchange[i])
    save_path = f'/Data/datasets/Option order results/{args.dataset}/LLM results/llama3/{args.subset}_results_{result_file_name}.jsonl'
